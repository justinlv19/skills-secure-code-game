...
class DB_CRUD_ops(object):
    ...
    def get_stock_info(self, stock_symbol):
        ...
        try:
            ...
            has_restricted_char = any([char in original for char in restricted_chars])
            # checks if input contains a wrong number of single quotes against SQL injection
            correct_number_of_single_quotes = str(query[:-1] + "'" + stock_symbol + "'").count("'") == 2
    ...
    def update_stock_price(self, stock_symbol, price):
        ...
        try:
            ...
            res += "[QUERY] " + query.replace("?", "'" + str(int(price)) + "'", 1)[:-1] + "'" + stock_symbol + "'" + "\n"
